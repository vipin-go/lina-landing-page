...
- `landingPage.design` (optional, object) — explicitly selects the renderer's visual treatment; presentation is never inferred from the persona name.
  - `variant` (optional, `default`, `signature`, `banking`, `form-operations`, `cinematic-campaigns`, or `recruiting-operations`) — `signature` enables the editorial layout; `banking` enables the navy, grid-led governed-banking presentation; `form-operations` enables the simulation-first Sherma information architecture; `cinematic-campaigns` enables Archer's video-led campaign studio; `recruiting-operations` enables Lina's public-signal recruiting presentation and requires the complete bounded `landingPage.recruitingOperations` object.
